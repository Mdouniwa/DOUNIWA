...
すみれラベンダー・ミント・ティアラゴールドを脇役に。

装飾方針(v6改訂): 開発時に生成した静的アート素材(`public/fairy/*.png`,
`public/art/**`)の使用を許可する。3Dレンダリング調・森と木漏れ日・緑/金/淡青基調。
ランタイムでの画像生成API呼び出しはゼロを維持(素材は必ずリポジトリに同梱)。
パスは `src/lib/artAssets.ts` に集約し、ハードコードの重複を禁止。
アート背景を敷く画面は `.has-art-bg` + `<ArtBg>`(CSSの角花飾りは自動で消える)。
新たな装飾が必要でアート素材が過剰な場合は、従来どおりCSSのみの装飾も可。
