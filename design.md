...
- 既存アニメーション(ページめくり520ms・録音パルス・生成オービット・紙吹雪)は保持。
- 新規モーションは追加しない。`prefers-reduced-motion: reduce` で全アニメーションを
  実質停止(index.css のグローバルフォールバック)。
